...
# logger = logging.getLogger("flarestack")
...
def make_band_mask(mh_dict):
    ...
    if 'injection_declination_bandwidth' not in mh.inj_dict.keys():
        logger.info("Injection declination bandwidth not specified, setting default value 1.5")
    ...
    for i in mh.seasons.values():
        cats, paths, mask_indices, source_indices = band_mask_cache_name(i, sources, injection_bandwidth)
        
        if np.sum([not os.path.isfile(x) for x in paths]) > 0.0:
            logger.info("No band masks found for season {0}, running 1 trial to write them".format(i.season_name))
