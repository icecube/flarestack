...
from flarestack.analyses.skylab_crosscheck.skylab_crosscheck_with_same_scramble import make_sources
from flarestack.analyses.skylab_crosscheck.data import get_skylab_crosscheck_stacking_same_sources_res
import math
...
if cluster and (cluster > ntrials):
    raise ValueError(f'Number of trials {ntrials} is smaller than number of cluster jobs {cluster}!')
...
if __name__ == '__main__':
    ...
    # making plots
    sl_res = get_skylab_crosscheck_stacking_same_sources_res("manualmedianPointSourceTracks_v002p01", seasons)
    gamma_color = {2.: 'blue'}
    for hsphere, gamma_res in stacked_sens_flux.items():

        fig, (ax, ax2) = plt.subplots(2, gridspec_kw={'height_ratios': [3, 1]}, sharex='all')

        for gamma, sens_dict in gamma_res.items():

            yerr = np.atleast_2d(sens_dict['sens_e']).T
            logger.debug(f"error shape is {np.shape(yerr)}")
            ax.errorbar(['flarestack'], sens_dict['sens'], yerr=yerr, marker='o',
                        color="blue")
            y1_fs = sens_dict['sens'] - sens_dict['sens_e'][0]
            y2_fs = sens_dict['sens'] + sens_dict['sens_e'][1]
            ax.fill_between(['flarestack', 'skylab'], y1=y1_fs, y2=y2_fs, color='blue', alpha=0.2)

            for kernel in [1]:

                if sl_res:
                    this_sl_res = sl_res[kernel][hsphere][gamma]
                    ax.errorbar(['skylab'], this_sl_res['sens'], yerr=this_sl_res['sens_e'], marker='o',
                                color="red", alpha=0.5+kernel/2)
                    y1_sl = this_sl_res['sens'] - this_sl_res['sens_e']
                    y2_sl = this_sl_res['sens'] + this_sl_res['sens_e']
                    ax.fill_between(['flarestack', 'skylab'], y1=y1_sl, y2=y2_sl,
                                    color='red', alpha=0.2)

                    ax2.errorbar(['flarestack'], sens_dict['sens']/this_sl_res['sens'], yerr=yerr/this_sl_res['sens'],
                                 marker='o', color='blue')
                    ax2.fill_between(['flarestack', 'skylab'],
                                     y1=(sens_dict['sens'] - yerr[0])/this_sl_res['sens'],
                                     y2=(sens_dict['sens'] + yerr[1])/this_sl_res['sens'],
                                     color='blue', alpha=0.2)

                    ax2.errorbar(['skylab'], this_sl_res['sens'] / this_sl_res['sens'],
                    ...
                    logger.warning(f'No SkyLab results for gamma={gamma:.2f} and kernel {kernel}'
