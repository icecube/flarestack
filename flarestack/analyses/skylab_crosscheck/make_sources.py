...
if __name__ == '__main__':
    ...
    for dir in [output_dir_for_fs(), output_dir_for_sl()]:
        logging.info('saving {0} sources to {1}'.format(len(sources), dir + 'sources.npy'))
        np.save(dir + 'sources.npy', sources)

    for n in nsources:
        logging.info('saving {0} sources to {1}'.format(len(sources[:n]), output_dir_for_fs() + str(n) + 'sources.npy'))
    ...
    for sindec in same_sindecs:
        ...
        for n in nsources:
            logging.info('saving {0} sources to {1}'.format(len(sources[:n]), output_dir_for_fs(sindec) + str(n) + 'sources.npy'))
