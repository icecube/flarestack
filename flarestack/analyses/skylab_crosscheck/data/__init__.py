...
def get_skylab_crosscheck_stacking_same_sources_res(dataset, seasons, disc=False):
    path = f'{os.environ["SKYLAB_SCRATCH_DIR"]}/data/fs_crosscheck_with_same_scrambles/stacking_sensitivity_nsources_' \
           f'{dataset}_'
    if seasons == 'all':
        path += 'all'
    else:
...
def get_skylab_crosscheck_stacking_same_sources_ts_array(dataset, seasons, kernel, gamma, hsphere,
                                                         bgk=False, disc=False):
    path = f'{os.environ["SKYLAB_SCRATCH_DIR"]}/data/storage/fs_crosscheck_with_same_scrambles/' \
...
def vary_sindecs(original_sindecs, step, increase_param):
    ...
    for i, orig_sindec in enumerate(original_sindecs):
        new_sindecs.append(orig_sindec + (-1)**i * increase_param * step * i/len(original_sindecs))
    return new_sindecs
...
def make_varying_sindecs_sources(sindec, step, fn, **kwargs):
    ...
    new_sindecs = vary_sindecs(np.sin(original_sources['dec_rad']), step,
