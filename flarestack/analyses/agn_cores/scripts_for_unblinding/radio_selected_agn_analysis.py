"""Script to compare the sensitivity and discovery potential for the Radio-selected AGN sample
as a function of injected spectral index. The analysis is performed for 10 sub-samples of the
X-ray brightest sources of the total Radio-selected AGN sample (total of 9749 sources).
"""
...
nr_brightest_sources = [1, 3, 10, 31, 100, 316, 1000, 3162, 9749]
...
for (cat_key, res_dict) in all_res.items():
    ...
    for (nr_srcs, res) in iter(res_dict.items()):
        ...
        for i, sens_dp in enumerate([sens_livetime, disc_pots_livetime]):
            ...
            ax1.set_xlabel(
                r"Integrated X-Ray Flux [erg cm$^{-2}$ s$^{-1}$]", fontsize=12
            )
            ax1.set_xscale("log")
            ax1.set_yscale("log")
            ax1.yaxis.set_major_formatter(ScalarFormatter(useMathText=True))
        ...
        for i, sens_dp in enumerate([ratio_sens, ratio_disc]):
            ...
            plt.tight_layout()
            plt.savefig(
                plot_output_dir(base_dir) + labels[i + 2] + "_vs_Nsrcs_ratio.pdf"
            )
            plt.close()
        ...
        for i, sens_dp in enumerate([ratio_sens, ratio_disc]):
            ...
            plt.tight_layout()
            plt.savefig(
                plot_output_dir(base_dir) + labels[i + 2] + "_vs_frac_ratio.pdf"
            )
            plt.close()
        ...
        for i, sens_dp in enumerate([ratio_sens, ratio_disc]):
            ...
            plt.tight_layout()
            plt.savefig(
                plot_output_dir(base_dir) + labels[i + 2] + "_vs_Xray_cut_ratio.pdf"
            )
            plt.close()
        ...
        for i, sens_dp in enumerate([ratio_sens, ratio_disc]):
            ...
            ax1.plot(
                fracs,
                np.array(sens_dp) / saturate_ratio,
                marker="o",
                ls="",
                label="Ratio " + labels[i],
            )
            ...
            # ax1.legend(loc='upper left', framealpha=1)
            ax1.set_ylabel(
                labels[i] + r" (cumulative population $\nu$ flux/diffuse $\nu$ flux)",
                fontsize=10,
            )
            ...
            plt.tight_layout()
            plt.savefig(
                plot_output_dir(base_dir) + labels[i + 2] + "_vs_frac_diffuse.pdf"
            )
            plt.close()
        ...
        for i, sens_dp in enumerate([ratio_sens, ratio_disc]):
            ...
            plt.tight_layout()
            plt.savefig(
                plot_output_dir(base_dir) + labels[i + 2] + "_vs_Xray_cut_diffuse.pdf"
            )
            plt.close()
        ...
        for i, sens_dp in enumerate([ratio_sens, ratio_disc]):
            ...
            ax1.set_xlabel(
                r"Integrated X-Ray flux of all stacked sources [erg cm$^{-2}$ s$^{-1}$]"
            )
            ax1.yaxis.set_major_formatter(ScalarFormatter(useMathText=True))
            ax1.grid(True, which="both")
        ...
        for i, sens_dp in enumerate([ratio_sens, ratio_disc]):
            ...
            # Residual plot
            frame2 = fig1.add_axes((0.1, 0.1, 0.8, 0.19))
            plt.plot(
                int_xray_flux_erg_mask,
                n_src_mask,
                ls="-",
            ...
            frame2.yaxis.set_major_formatter(
                ticker.FuncFormatter(lambda y, _: "{:g}".format(y))
            )
            frame2.tick_params(
                axis="x",  # changes apply to the x-axis
            ...
            fig1.savefig(
                plot_output_dir(base_dir)
                + labels[i + 2]
                + "_vs_Xray_cut_diffuse_double_plot.pdf"
            )
            plt.close()
        ...
        frame1 = fig1.add_axes((0.1, 0.3, 0.8, 0.6))
        plt.plot(
            int_xray_flux_erg_mask_dp,
            dp_plot / saturate_ratio,
            ls="-",
            lw=2,
            # marker = 'o',
        ...
        plt.plot(
            int_xray_flux_erg_mask_sen,
            sens_plot / saturate_ratio,
            color="C0",
            ls="-.",
            lw=2,
            # marker = 'o',
        ...
        fig1.tight_layout()
        fig1.savefig(
            plot_output_dir(base_dir) + "sens_dp_vs_Xray_cut_diffuse_double_plot.pdf"
        )
        plt.close()
