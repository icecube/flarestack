...
from flarestack.analyses.ccsn.stasik_2017.ccsn_limits import get_figure_limits, limits
...
for gamma in gammas:
    ...
    for time_key, time_res in cat_res.items():
        ...
        this_sens.append(
            (float(time_key), this_rh.sensitivity, this_rh.sensitivity_err)
        )
    ...
    fig, ax = plt.subplots()
    ax.plot(
        this_sens_time[:, 0], this_sens_time[:, 1], color="orange", ls="--", marker="o"
    )
    ax.set_xlabel(r"$\Delta T$")
