...
if __name__ == "__main__":
    ...
    for cat in ["IIp", "IIn"]:
        ...
        for llh_time in time_pdfs:
            ...
            time_key = (
                str(llh_time["post_window"] + llh_time["pre_window"])
                if pdf_type == "box"
    ...
    for gamma in gammas:
        ...
        stacked_sens = {
            cat_name: np.array(
                [
                    (
                        stacked_sens_flux[cat_name][time_key][gamma][2],
                        float(time_key) / 364.25,
                    )
        ...
        for cat_name in stacked_sens:
            ...
            ax.plot(
                ...
                label=cat_name + " new sensitivity",
                ls="",
            )
        ...
        plt.savefig(
            plot_output_dir(
                raw
                + f"stacked_sensitivity_gamma{gammas[0]}_{mh_name}{plot_results}.pdf"
            )
        )
        plt.close()
