...
import argparse
import numpy as np
import math, pickle, os, logging, time, shutil, json
...
energy_range_sens_filename = os.path.join(storage_dir, base_raw, "differential_sens.pkl")

# define ranges of min/max energy to test
# e_range = np.logspace(2, 7, 12)
energies = np.logspace(2, 7, 11)
energy_intervals = [(energies[i], energies[i+1]) for i in range(len(energies) - 1)]
            ...
            for llh_time in time_pdfs:
                ...
                for gamma in gammas:
                    ...
                    for elow, ehigh in energy_intervals:
                        ...
                        ename = f"elow{elow:.1e}_ehigh{ehigh:.2e}"
                        full_e_name = f"{full_name}/differential_sens/{ename}/"
                        ...
                        mh_dict = {
                            ...
                            "llh_dict": llh_dict,
                            "scale": scale,
                            "n_trials": 100 / cluster if cluster else 10,
                            "n_steps": 10,
        ...
        for cat, cat_res in full_res.items():
            ...
            for time_key, time_res in cat_res.items():
                ...
                for gamma, ebound_res in time_res.items():
                    ...
                    for ename, mh_dict in ebound_res.items():
                        ...
                        debug_dict = {k: v for k, v in sens_mh_dict.items() if is_jsonable(v)}
                        ...
                        try:
                            ...
                            if rh.extrapolated_sens:
                                logging.warning(
                                    f"Extrapolated sensitivity for {ename}!"
                                )
                                gamma_sens[ename] = tuple()
                        ...
                        except (FileNotFoundError, OverfluctuationError) as e:
                            logging.warning(f"{ename}: {e}!")
                            gamma_sens[ename] = tuple()
