...
from flarestack.analyses.ccsn.necker_2019.unblinding_scripts.p_value_distribution.generate_background_pvalue_distribution_single \
    import single_filename, run_background_pvalue_trials, p_value_filename, p_value_directory
...
class MyPValueSubmitter(DESYSubmitter):
    ...
    def submit_cluster(self, param_dict):
        ...
        submit_cmd += f"-t 1-{n_tasks}:1 {MyPValueSubmitter.submit_file} {self.trials_per_task}"
...
if __name__ == '__main__':
    ...
    n_cpu = 30
    use_cluster = True  # NOTE: So far this is only implemented to run on the DESY cluster.
    #                           If you are on a different cluster, you can only run trials locally
    #                           (set use_cluster to False)

    sub = MyPValueSubmitter(param_dict=p_dict, use_cluster=use_cluster, n_cpu=n_cpu, **cluster_kwargs)
    sub.analyse()
