...
from flarestack.shared import flux_to_k, scale_shortener, storage_dir, pickle_dir, inj_param_dir
from flarestack.icecube_utils.reference_sensitivity import reference_sensitivity
from flarestack.analyses.ccsn.necker_2019.ccsn_helpers import sn_cats, updated_sn_catalogue_name, \
    sn_time_pdfs, raw_output_dir, pdf_names
...
energy_ranges = {
    2.: {
        'e_min_gev': np.logspace(3, 4.5, 6),
        'e_max_gev': np.logspace(5.5, 7, 6)
    },
    2.5: {
        'e_min_gev': np.logspace(2, 3.5, 6),
        'e_max_gev': np.logspace(5, 6, 6)
    }
}
                ...
                for gamma, ebound_res in time_res.items():
                    ...
                    for ebound_key, erange_res in ebound_res.items():
                        ...
                        for ebound, mh_dict in erange_res.items():
                            ...
                            wrong_name = raw_output_dir + f'{base_raw}/{pdf_type}/{cat}/{time_key}/{gamma}/{ebound_key}/{ebound:.4f}/'
                            ...
                            if (pdf_type == 'decay') and (gamma == 2.):
                                gamma_str = '2'
                            else:
                                gamma_str = str(gamma)

                            sens_name = f"{base_raw}/{pdf_type}/{cat}/{time_key}/{gamma_str}"
                            sens_mh_dict = dict(mh_dict)
                            sens_mh_dict['name'] = sens_name
                            logging.info('----- loading sensitivity results -------')
                            sens_rh = ResultsHandler(sens_mh_dict, do_disc=False, do_sens=False)
                            ...
                            try:
                                logging.info('----------- loading energy range calculations ------------')
                                # rh = ExperimentalResultHandler(mh_dict, do_sens=False, do_disc=False)
                                rh = ResultsHandler(mh_dict, do_sens=False, do_disc=False)

                                # insert the background trials
                                logging.debug('setting background TS distribution from sensitivity results')
                                ...
                                logging.warning(f'OverfluctuationError for {ebound}: {e}!')
