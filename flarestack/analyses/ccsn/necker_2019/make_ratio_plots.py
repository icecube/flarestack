...
for pdf_type in ["box", "decay"]:
    ...
    stacked_sens_necker = {
        cat_name: np.array(
            [
                ...
                for time_key in necker_sens_dict[cat_name]
            ],
            dtype="<f8",
        )
    ...
    stacked_sens_stasik_flaresatck = {
        cat_name: np.array(
            [
                ...
                for time_key in stasik_sens_flaresatck_dict[cat_name]
            ],
            dtype="<f8",
        )
    ...
    xlabel = (
        "Box function $\Delta T$ [d]"
        if "box" in pdf_type
        else "t$_{\mathrm{decay}}$  [y]"
    )
    ...
    for cat_name in stacked_sens_necker:
        ...
        stasik_arr = np.array(
            stacked_sens_stasik_flaresatck[cat_name if not "P" in cat_name else "IIp"],
            dtype="<f8",
        )

        plot_arr[:, 0][np.argsort(plot_arr[:, 1])] /= stasik_arr[:, 0][
            np.argsort(stasik_arr[:, 1])
        ]  # res_stasik['E'][np.argsort(res_stasik['t'])]
    ...
    fname = (
        f"{plot_dir}/stacked_sensitivity_ratio_reproduced_gamma{gamma}_{pdf_type}.pdf"
    )
    ...
    for cat_name in stacked_sens_necker:
        ...
        # yerr = plot_arr[:, 0] * 0.1
        pval_mask = (
            p_vals[pdf_type][cat_name if not "P" in cat_name else "IIp"]["pval"] >= 0.5
        )
