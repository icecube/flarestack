...
import numpy as np
import logging
from flarestack.shared import log_dir
...
def wait_for_cluster(job_ids=None):
    """
    Calls the wait_for_job function either for all submitted jobs, if no job ids are given,
    or for each job in the input separately
    :param job_ids: list_like, optional, list of job IDs to wait for
    """
    if not job_ids:
        wait_for_job()
    else:
...
def submit_to_cluster(path, n_cpu=1, n_jobs=10):
