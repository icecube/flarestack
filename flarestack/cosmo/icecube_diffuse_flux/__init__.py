...
def load_fit(fit):
    if fit == "joint":
        logger.warning(
            "Fit 'joint' was used, without a specified year. "
    ...
    if fit == "northern_tracks":
        logger.warning(
            "Fit 'northern_tracks' was used, without a specified year. "
...
def get_diffuse_flux(e_gev=1.0, fit="joint_15"):
...
def get_diffuse_flux_contour(fit="joint_15", contour_name="68"):
    ...
    if contour_name in [68.0, 68, 0.68, "68", "0.68", "68%"]:
        contour = contour_68
    elif contour_name in [95.0, 95, 0.95, "95", "0.95", "95%"]:
        contour = contour_95
    else:
        raise Exception(
            f"Contour '{contour_name}' not recognised. Please select '68' or '95'."
        )
...
def plot_diffuse_flux(fit="joint_15"):
    ...
    for i, contour in enumerate([68.0, 95.0]):
        all_c = [contour_68, contour_95][i]

        best_f, upper_f, lower_f, e_range = get_diffuse_flux_contour(
            fit=fit, contour_name=contour
        )

        plt.plot(e_range, e_range**2 * best_f(e_range), alpha=1.0, color="k")
