...
class TestUtilCosmo(unittest.TestCase):
    ...
    def test_get_diffuse_flux(self):
        ...
        for fit in all_fits:
            ...
            expected_1_gev = res_100_tev[0] * (10.0**5) ** res_100_tev[1]

            res_1_gev = get_diffuse_flux_at_1GeV(fit)

            self.assertEqual(expected_1_gev, res_1_gev[0])

            ratio = (res_1_gev[0] / get_diffuse_flux(1.0, fit=fit)[0]).to("").value
        ...
        for i, fit in enumerate(fits):
            ...
            ratio = (
                (res_100_tev[0] / get_diffuse_flux(10.0**5, fit=fit)[0]).to("").value
            )
            self.assertAlmostEqual(ratio, 1.0, places=3)
    ...
    def test_neutrino_cosmology(self):
        ...
        e_pdf_dict = {
            ...
            "source_energy_erg": 1.0e48 * u.erg,
            "gamma": bestfit[1],
        }
        ...
        res = calculate_transient_cosmology(
            e_pdf_dict, ccsn_rate, "test_CCSN", zmax=8.0, diffuse_fit=fit
        )
